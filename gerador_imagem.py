...
def _desenhar_selo_prime(draw, canvas_w: int, y_topo: int) -> None:
    """Desenha selo azul 'Exclusivo Prime' no canto superior direito."""
    largura_selo = 220
    altura_selo  = 44
    x_selo       = canvas_w - largura_selo - 16
    y_selo       = y_topo

    draw.rounded_rectangle(
        [x_selo, y_selo, x_selo + largura_selo, y_selo + altura_selo],
        radius=8,
        fill=AZUL_PRIME
    )

    fonte = _carregar_fonte(20, negrito=True)
    texto = "Exclusivo Prime"
    caixa = draw.textbbox((0, 0), texto, font=fonte)
    tx = x_selo + (largura_selo - (caixa[2]-caixa[0])) // 2
    ty = y_selo + (altura_selo  - (caixa[3]-caixa[1])) // 2
    draw.text((tx, ty), texto, font=fonte, fill=BRANCO)
...
def _carregar_logo(tamanho: int = 80):
    caminhos = [
        "logo.png",
...
def _adicionar_logo(canvas: Image.Image, tamanho: int = 80,
                    margem: int = 20,
                    posicao: str = "inferior_direito") -> Image.Image:
    ...
    fundo       = Image.new("RGBA", (logo.width + 16, logo.height + 16), (255, 255, 255, 200))
...
def criar_imagem_quadrada(titulo, preco_anterior, preco_promo,
    ...
    if tipo_deal == "PRIME_EXCLUSIVE":
        _desenhar_selo_prime(draw, W, 165)
...
# ─── FORMATO 2 — Pinterest (1000×1500px) ────────────────────────
...
def criar_imagem_pinterest(titulo, preco_anterior, preco_promo,
    ...
    if tipo_deal == "PRIME_EXCLUSIVE":
        _desenhar_selo_prime(draw, W, 182)
...
# ─── FORMATO 3 — Story / TikTok (1080×1920px) ───────────────────
...
def criar_imagem_story(titulo, preco_anterior, preco_promo,
    ...
    if tipo_deal == "PRIME_EXCLUSIVE":
        _desenhar_selo_prime(draw, W, 332)
